...
When working on OpenEdge health checks, diagnostics, monitoring, buffers, locks, or performance analysis, prefer the `hck-ai-tools` skill for the `openedge-hck` MCP tools, headless launcher CLI, backend HTTP API, diagnostic spec files, ports, and settings.
