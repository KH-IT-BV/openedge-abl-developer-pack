// Standalone smoke test for the bundled dictionary MCP server.
//
// Spawns mcp-servers/dictionary/index.cjs (its own bundled launcher + OE backend +
// dictionary.pl), speaks MCP over stdio with a tiny dependency-free JSON-RPC client,
// and runs real tool calls against a live database using an inline connectionString.
//
// Usage (PowerShell):
...
const serverEntry = path.join(HOME, 'dictionary', 'index.cjs');
const env = {
  ...process.env,
  DLC,
  DICTIONARY_MCP_LAUNCHER_PATH: path.join(HOME, 'dictionary', 'native', 'launcher', 'dictionaryLauncherCli.js'),
  DICTIONARY_MCP_BACKEND_PATH: path.join(HOME, 'dictionary', 'dictionary_backend'),
  // Alternate ports so we never collide with an already-running backend.
  DICTIONARY_MCP_HTTP_PORT: process.env.DICTIONARY_MCP_HTTP_PORT || '23404',
  DICTIONARY_MCP_ABL_SOCKET_PORT: process.env.DICTIONARY_MCP_ABL_SOCKET_PORT || '23402',
  DICTIONARY_MCP_STARTUP_TIMEOUT_MS: '60000',
};
...
async function main() {
  ...
  const tablesRes = await rpc('tools/call', {
    name: 'list_tables',
    arguments: { database: DB, connectionString: CONN },
  });
  ...
  console.log('\nPASS: bundled dictionary MCP server works standalone.');
